...
public class CpuWasteDetector implements IDetector {
    ...
    private long estimatedTimeSpentInJobs() {
        ...
        for (Map.Entry<Integer, SparkJob> jobs : this.replaySparkEventLogs.getJobs().entrySet()) {
    ...
    private long getMaxConcurrent() {
        ...
        this.replaySparkEventLogs.getExecutors().values().forEach(sparkExecutor -> {
            long correctedEndTime;
            if (sparkExecutor.getRemoveTimestamp() == 0) {
                if (this.replaySparkEventLogs.getApplication().getAppEndTimestamp() == 0) {
                    correctedEndTime = System.currentTimeMillis();
                } else {
                    correctedEndTime = this.replaySparkEventLogs.getApplication().getAppEndTimestamp();
                }
            } else {
    ...
    private long getInJobComputeMillisUsed() {
        long sum = 0L;
        for (Map.Entry<Integer, SparkJob> entry : this.replaySparkEventLogs.getJobs().entrySet()) {
